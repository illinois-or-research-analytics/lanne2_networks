...
def main(edge_input: str = typer.Option(..., "--filepath", "-f"),
    ...
    if out_edge_file == "":
        out_edge_file = edge_input.replace("SBM_mcs","SBM_mcs_plusEdges_v2")
    else:
        out_edge_file = out_edge_file.replace("SBM_mcs","SBM_mcs_plusEdges_v2")
    ...
    log_path = f'SubNetworks_SBMMCS_plusedges_logs_v2/{net_name}.log'
    ...
    try:
        ...
        ## emp_node_mapping = {node_id : node_iid}
        ## emp_node_mapping_reversed = {node_iid : node_id}
        # emp_node_mapping_reversed = {u:str(v) for v,u in emp_node_mapping.items()}
        ...
        while continue_iterations and iteration_count<=1:
            ...
            # # Set all diagonal values to zero to avoid intra cluster edges
            ...
            clusters_to_be_considered = set(np.unique(avail_nodes_block_assignment))
