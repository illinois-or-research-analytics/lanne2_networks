...
def main(edge_input: str = typer.Option(..., "--filepath", "-f"),
    ...
    if out_edge_file == "":
        out_edge_file = f'SBM_OutlierCluster_samples/{net_name}/N_graph_edge_list.tsv'
    else:
        out_edge_file = out_edge_file
    
    if out_node_file == "":
        out_node_file = f'SBM_OutlierCluster_samples/{net_name}/N_graph_node_list.tsv'
    else:
    ...
    logging.basicConfig(filename=f'SBM_OutlierCluster_samples/{net_name}/SBM_OutlierCluster_samples.log', filemode='w', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
